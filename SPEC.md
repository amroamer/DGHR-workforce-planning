...
## 13. AI LAYER (never blocking; live-or-fallback, always labeled)

services/ai_service.py: if ANTHROPIC_API_KEY set and DEMO_AI_MODE != "fallback" → Anthropic Messages API (MODEL_NAME, default current Claude Sonnet), 10s timeout (25s for the review brief — the one full-deliverable generation), thinking disabled, no sampling params (current Claude models reject `temperature`); on any error/timeout → services/fallbacks.py deterministic canned output. Demo indistinguishable offline. Frontend always shows 1.5–3s "✨ Analyzing…". Voice per §4.4. Original three features: Driver AI Summary (POST /ai/driver-summary/{entity} → 150–200-word structured narrative), Anomaly narrative (POST /ai/anomaly-narrative/{id} → 3–4 sentences + 2 checks, cached), Job-title mapping (POST /ai/map-titles → best title + family + confidence; AI wins ties vs fuzzy).

**§13.1 — Post-MVP expansion (deliberate departure from "exactly three", July 2026).** Same contract for every addition — live-Anthropic-or-deterministic-fallback, `source` field in every response, all figures from computed DB aggregates, never blocking: Smart Assist (POST /planning/smart-assist → drivers + floors proposed from a free-text/voice note; regex fallback), Report narrative (POST /ai/report-narrative → entity/gov executive summary from the report aggregates), Clarification drafting (POST /ai/draft-clarification → DGHR question or entity reply grounded in the element's submitted figures; template fallback with [bracketed placeholders], never fabricated evidence), Review brief (POST /ai/review-brief/{sub} → reviewer copilot: summary + risks + first checks from sizing, version diff, review state and same-typeset peer medians; on demand, never cached), Ask-the-data chat (POST /ai/ask + /ai/ask/stream → grounded Q&A over the Government-Wide Position OR one entity's position (scope=gov|entity); the model only ever sees a snapshot of the same figures the page renders, so it cannot invent a number; answers stream token-by-token (NDJSON); keyword-intent deterministic fallback; ask by text or voice). Voice is EN-AE/AR-AE selectable via a shared VoiceLangToggle wired into every mic (Stepper voice note + reply, DGHR clarify/query, chat). Voice capture is client-side Web Speech API (no backend, no new deps) feeding Smart Assist and clarification replies; unsupported browsers fall back to the §12 sample transcript.
